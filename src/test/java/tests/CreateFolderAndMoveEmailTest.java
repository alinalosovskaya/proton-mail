...
public class CreateFolderAndMoveEmailTest extends BaseTest {
    ...
    private static final String FOLDER_NAME = "TestFolder_" + System.currentTimeMillis();

    /** Unique email subject for every test run. */
    private static final String EMAIL_SUBJECT = "QA Automation " + System.currentTimeMillis();


    private static final String EMAIL_BODY = "This email was created by the automation test.";
    ...
    public void createFolderAndMoveEmailIntoIt() {
        ...
        /* 2. Create email */
        ComposePage compose = sidebar.startNewMessage();

        compose.addressTo(TEST_EMAIL).giveSubject(EMAIL_SUBJECT).writeBody(EMAIL_BODY).waitUntilContentEntered(
                        TEST_EMAIL,
                        EMAIL_SUBJECT,
                        EMAIL_BODY);
        ...
        Assert.assertTrue(inbox.waitForMail(EMAIL_SUBJECT),"Inbox should contain the newly sent email: '" +
                        EMAIL_SUBJECT +  "'");

        /* 5. Create folder */
        sidebar = sidebar.createFolder(FOLDER_NAME);

        /* 6. Verify folder was created */
        Assert.assertTrue( sidebar.isFolderListed(FOLDER_NAME),"New folder '" + FOLDER_NAME +
                        "' should appear in the sidebar" );

        /* 7. Return to Inbox */
        inbox = sidebar.goToInbox();

        /* 8. Select email nad move to folder */
        inbox.selectMailByCheckbox(EMAIL_SUBJECT ).moveSelectedMailTo(FOLDER_NAME);
